...
export default function JobFormPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <h1 className="mb-6 text-2xl font-bold text-gray-900">{isEdit ? 'Edit Job' : 'Post a Job'}</h1>

      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
        <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <h2 className="mb-4 font-semibold text-gray-800">Job Details</h2>
          <div className="flex flex-col gap-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="flex flex-col gap-1">
                ...
                <select {...register('jobType')} className="rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div className="flex flex-col gap-1">
              ...
              <textarea {...register('description')} rows={5} placeholder="Describe the role, responsibilities, requirements..." className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
        ...
        <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <h2 className="mb-4 font-semibold text-gray-800">Required Skills</h2>
          ...
          {Object.keys(requiredSkills).length > 0 && (
            <div className="mt-4 flex flex-col gap-2">
              <p className="text-sm font-medium text-gray-700">Required level (1–5):</p>
                  ...
                  <div className="flex gap-1">
                    {[1, 2, 3, 4, 5].map((v) => (
                      <button key={v} type="button" onClick={() => setRequiredSkills((p) => ({ ...p, [s.id]: v }))}
                        className={`h-7 w-7 rounded-full text-xs font-medium transition ${requiredSkills[s.id] >= v ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-500 hover:bg-blue-100'}`}>
                        {v}
                      </button>
                    ))}
                  </div>
