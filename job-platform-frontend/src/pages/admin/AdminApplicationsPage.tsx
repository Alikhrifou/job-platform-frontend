  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <h1 className="mt-1 text-2xl font-bold text-gray-900 dark:text-white">{t('admin.allApplications')}</h1>
      </div>

      {loading ? (
        <p className="text-sm text-gray-500 dark:text-slate-400">Loading…</p>
      ) : apps.length === 0 ? (
        <p className="text-sm text-gray-500 dark:text-slate-400">{t('admin.noApplications')}</p>
      ) : (
          ...
          <table className="w-full text-left text-sm">
            <thead className="border-b bg-gray-50 dark:bg-slate-800 text-xs uppercase text-gray-500 dark:text-slate-400">
              <tr>
                ...
                <th className="px-4 py-3">{t('admin.student')}</th>
                <th className="px-4 py-3">{t('admin.job')}</th>
                <th className="px-4 py-3">{t('admin.company')}</th>
                <th className="px-4 py-3">{t('admin.status')}</th>
                <th className="px-4 py-3">{t('student.match')}</th>
                <th className="px-4 py-3">{t('student.applied')}</th>
              </tr>
            </thead>
