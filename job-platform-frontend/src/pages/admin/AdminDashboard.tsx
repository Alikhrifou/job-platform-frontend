...
export default function AdminDashboard() {
  ...
  const cards = [
    ...
    },
    {
      ...
      description: t('admin.skillsDesc'),
      to: '/admin/skills',
  ...
  return (
    <div>
      {/* Header */}
      <div className="mb-8">
        <p className="text-sm font-medium text-indigo-600">{t('admin.portal')}</p>
        <h1 className="mt-1 text-3xl font-bold text-slate-900 dark:text-white">{t('admin.dashboard')}</h1>
      ...
      <div className="mb-8 rounded-2xl border border-amber-200 bg-gradient-to-r from-amber-50 to-orange-50 p-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-amber-800">{t('admin.adminAccess')}</p>
            <p className="text-xs text-amber-600">{t('admin.fullControl')}</p>
          </div>
        </div>
      </div>
